...
RUN rpm -Uvh https://www.mirrorservice.org/sites/dl.fedoraproject.org/pub/epel/7/x86_64/e/epel-release-7-5.noarch.rpm \
  && yum -y install passwd sudo git wget openssl ; yum clean all
...
RUN yum install -y erlang \
  ...
  && yum install -y rabbitmq-server-3.6.1-1.noarch ; yum clean all
...
RUN systemctl stop rabbitmq-server ; rabbitmq-plugins enable rabbitmq_management --offline
...
# uchiwa
RUN yum install -y uchiwa ; yum clean all
